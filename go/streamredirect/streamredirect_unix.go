...
func prepareStreamRedirectTarget(targetFD int, path string) (*preparedStreamRedirect, error) {
	...
	file, err := openStreamRedirectFile(path)
	if err != nil {
		return nil, err
	}
...
func openStreamRedirectFile(path string) (*os.File, error) {
	info, err := os.Lstat(path)
	if err == nil {
		if info.Mode()&os.ModeSymlink != 0 {
			return nil, errors.New("stream redirection target must not be a symbolic link")
		}
		if !info.Mode().IsRegular() {
			return nil, errors.New("stream redirection target must be a regular file")
		}
		fd, err := syscall.Open(path, syscall.O_WRONLY|syscall.O_APPEND|syscall.O_NOFOLLOW, 0)
		if err != nil {
			return nil, err
		}
	...
	fd, err := syscall.Open(
		path,
		syscall.O_WRONLY|syscall.O_APPEND|syscall.O_CREAT|syscall.O_EXCL|syscall.O_NOFOLLOW,
		0o600,
	)
	if err != nil {
		return nil, err
