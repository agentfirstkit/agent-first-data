...
func skillInstall(spec SkillSpec, opts SkillOptions) (SkillReport, *SkillError) {
	...
	for _, target := range targets {
		...
		if skillPathExists(target.skillPath) {
		...
		if e := os.WriteFile(target.skillPath, []byte(content), 0o644); e != nil {
			return nil, &SkillError{Message: fmt.Sprintf("write skill failed: %v", e)}
		}
...
func skillTargetStatus(spec SkillSpec, target skillTarget) (SkillTargetStatus, *SkillError) {
	installed := skillIsFile(target.skillPath)
	...
	if installed {
		...
		current = skillNormalizeText(spec, text) == skillNormalizeText(spec, spec.Source)
		if e := skillValidateFrontmatter(text); e != nil {
			msg := e.Error()
			validationError = &msg
		} else {
			valid = true
...
func skillManagedContents(spec SkillSpec) string {
	generatedBy := skillGeneratedBy(spec)
	marker := skillMarker(spec)
...
func skillValidateText(spec SkillSpec, text string) *SkillError {
	if err := skillValidateFrontmatter(text); err != nil {
		return &SkillError{
...
func skillValidateFrontmatter(text string) error {
	...
	if strings.TrimSpace(lines[0]) != "---" {
		return errors.New("missing opening --- YAML front matter delimiter")
	}
	foundEnd, hasName, hasDescription := false, false, false
	for idx := 1; idx < len(lines); idx++ {
		...
		if strings.HasPrefix(line, " ") || strings.HasPrefix(line, "\t") {
			return fmt.Errorf("line %d: nested YAML is not supported here", lineNo)
		}
		...
		if strings.Contains(value, ": ") {
			return fmt.Errorf("line %d: unquoted scalar contains ': '; quote the value", lineNo)
		}
	}
	if !foundEnd {
		return errors.New("missing closing --- YAML front matter delimiter")
	}
	if !hasName {
...
func skillIsManagedOrBundled(spec SkillSpec, path string) (bool, *SkillError) {
	if !skillPathExists(path) {
		return false, nil
	}
	data, err := os.ReadFile(path)
	if err != nil {
		return false, &SkillError{Message: fmt.Sprintf("read skill failed: %v", err)}
	}
...
func skillTextIsManagedOrBundled(spec SkillSpec, text string) bool {
	marker := skillMarker(spec)
	generatedBy := skillGeneratedBy(spec)
	if strings.Contains(text, marker) && strings.Contains(text, generatedBy) {
		return true
	}
...
func skillNormalizeText(spec SkillSpec, text string) string {
	...
	for _, line := range skillSplitLines(text) {
		trimmed := strings.TrimSpace(line)
		if strings.Contains(trimmed, marker) || strings.Contains(trimmed, generatedBy) {
			continue
		}
